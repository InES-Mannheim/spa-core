...
public class BPMN2Reads {
	public String individualNameSpace;
    ...
    public BPMN2Reads(String individualNameSpace) {
      this.individualNameSpace = individualNameSpace;
    }
    
  	public Model convertToRdf(BpmnModelInstance modelInstance) {
  		Preconditions.checkNotNull(modelInstance, "BPMN model must not be null.");
  		  final OntModel schemaModel = readSchema();
  		  final OntModel ontModelInstance = createInstanceModelAndAddItToSchema(schemaModel);
  		  final String blankSubnodeProperty = SCHEMA_NAMESPACE + "has_";
  		  
  		  for(Process process : findAllProcesses(modelInstance)){
	...
	public Collection<Process> findAllProcesses(BpmnModelInstance modelInstance) {
		return modelInstance.getModelElementsByType(Process.class);
	}

	public OntModel createInstanceModelAndAddItToSchema(final OntModel schemaModel) {
