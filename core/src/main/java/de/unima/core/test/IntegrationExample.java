...
public class IntegrationExample {
 ...
 // Create repository
    ...
    r.createProject(projectId, "BPMN2.0");
    
    Project p = r.getProject(projectId);
    
    // Create data pool in project
    String poolId = "SampleDataPool";
    p.createDataPool(poolId);
    
    DataPool dp = p.getDataPool(poolId);
    
    
    String targetNs = "http://spa.org/" + projectId + "/" + poolId + "#";
    
    // Read process model from example schema; redundant information: model importer
    IOObject<BPMN20File> processModel = new IOObjectImpl<BPMN20File>(new BPMN20FileImpl("tmp/example-spa.bpmn"), new BPMN20ImporterImpl(targetNs));
    ...
    OntModel m = processModel.getData();
